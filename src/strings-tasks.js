...
function countVowels(str) {
  ...
  for (let i = 0; i < temp.length; i += 1) {
    if (vowels.includes(temp[i])) sum += 1;
  }
  return sum;
}
...
function isPalindrome(str) {
  const reverseStr = str
    ...
    .reverse()
    .join('');
  return str.toLowerCase().replace(/[.,!? ]/g, '') === reverseStr;
}
...
function extractNameFromTemplate(value) {
  ...
  return `${newValue[newValue.length - 2]} ${newValue[newValue.length - 1]}`;
}
...
function encodeToRot13(str) {
  ...
  const rot13Upper = 'NOPQRSTUVWXYZABCDEFGHIJKLM';
  const rot13Lower = 'nopqrstuvwxyzabcdefghijklm';

  let total = '';

  for (let i = 0; i < str.length; i += 1) {
    if (str[i].toUpperCase() === str[i] && str[i].toLowerCase() !== str[i]) {
      total += rot13Upper[normalUpper.indexOf(str[i])];
    } else if (
      str[i].toLowerCase() === str[i] &&
      str[i].toUpperCase() !== str[i]
    ) {
      total += rot13Lower[normalLower.indexOf(str[i])];
    } else {
      total += str[i];
    }
  }
